...
import { getAppMockConf } from '@/server/service/mainService';
import { readObjectFromJsonFile } from '@/utils/jsonUtils';
...
process.on('message', (msg: any) => {
  const { apiPath, param, messageId } = msg;

  Promise.all([readObjectFromJsonFile(path.join(projectRootDir, 'mock', 'mockConf.json')), getAppMockConf()]).then(([mockConf, appMockConf]) => {
    const { memoryMockItemAndSceneItemConf, memoryMockItemAndSceneItemListPair } = appMockConf;
